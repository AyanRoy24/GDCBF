...
flags.DEFINE_string('mode_type', 'fisor', 'Mode for training')
...
def call_main(details):
    ...
    for i in trange(details['max_steps'], smoothing=0.1, desc=details['experiment_name']):
        ...
        if i % details['eval_interval'] == 0:
            # agent.save(f"./results/{details['experiment_name']}", save_time)
            # save_time += 1
            if details['env_name'] == 'PointRobot':
...
def main(_):
    ...
    parameters['mode_type'] = FLAGS.mode_type
    parameters['ratio'] = FLAGS.ratio
    parameters['group'] = parameters['env_name']
    parameters['experiment_name'] = str(FLAGS.env_id) + '_' + str(parameters['mode_type']) + '_' + str(parameters['env_name'])
    ...
    print(parameters)
    # if not os.path.exists(f"./results/{parameters['experiment_name']}"):
    #     os.makedirs(f"./results/{parameters['experiment_name']}")
    # with open(f"./results/{parameters['experiment_name']}/config.json", "w") as f:
