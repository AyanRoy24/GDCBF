...
def percentile_loss(diff, tau=0.6):
    """Percentile (pinball) loss: asymmetric L1, robust to outliers."""
...
class CBF(Agent):
    ...
    cost_ub: float
    r_min: float 
    mode:int = struct.field(pytree_node=False)  # 1: deterministic-smooth, 2: robust-smooth
    ...
    def create(
        ...
        cost_tau: float = 0.2,
        r_min: float = -1.0,
        mode: int = 1,  # 1: deterministic-smooth, 2: robust-smooth
    ...
    def update_h(agent, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        ...
        if agent.mode not in (1, 2):
            raise ValueError(f"Unknown CBF mode: {agent.mode}")

        # Smooth barrier update (Eq. 9): convex mix of the immediate cost with the
        # bootstrapped worst case. Shared by both arms; the arms differ only in the
        # regression loss used to fit Qh below.
        ...
        def Qh_loss(safe_critic_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            ...
            # TD
            if agent.mode == 2:  # robust: percentile regression approximates max over s'
                qh_loss = percentile_loss(target_qh - qhs, agent.transition_tau).mean()
            else:  # deterministic: symmetric L1
